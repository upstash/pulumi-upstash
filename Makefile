PROJECT_NAME := upstash Package

SHELL            := /bin/bash
PACK             := upstash
ORG              := upstash
PROJECT          := github.com/${ORG}/upstash-pulumi-provider
NODE_MODULE_NAME := @upstash/${PACK}
TF_NAME          := ${PACK}
...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/pulumi/pulumi-upstash,${REPOSITORY},g' provider/go.mod; \
